...
Safari.prototype.fetchBookById = function fetchBookById(id, username, password) {
  ...
  return this.authorizeUser(username, password).then( (accessToken) => {
    ...
    // --------------------------------------------
    // TODO: meta infos intéressantes à conserver !
    // --------------------------------------------
    // https://stackoverflow.com/questions/10729276/how-can-i-get-the-full-object-in-node-jss-console-log-rather-than-object
    // logger.log(util.inspect(meta, {showHidden: false, depth: null}));
    logger.log(`meta -> issued: ${meta['issued']}`);
    logger.log(`meta -> orderable_title: ${meta['orderable_title']}`);
    logger.log(`meta -> authors: ${util.inspect(meta['authors'], {showHidden: false, depth: null})}`);
    //
    var obj = {
        issued: meta['issued'],
        orderable_title: meta['orderable_title'],
        authors: util.inspect(meta['authors'], {showHidden: false, depth: null})
    };
    //
    var fs = require('fs');
    fs.writeFile(`./${id}.json`, JSON.stringify(obj), (err) => {
        if (err) {
            console.error(err);
            return;
        };
    ...
    // logger.log(util.inspect(toc, {showHidden: false, depth: null}));
